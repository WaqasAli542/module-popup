    ...
    return function (config) {
        if (config.isEnabled === "1") {
            var options = {
                ...
                innerScroll: true,
                title: '',
                modalClass: 'waqas-popup',
                buttons: [{
                    text: $.mage.__('' + config.buttonText),
                    class: 'waqas-popup-btn-action',
                    click: function () {
                        this.closeModal();
                    }
                }],
                ...
                closed: function () {
                    console.log('closed');
                    setCookie('waqas_custom_popup', 1, 1);
                    setNextPopOpenTime();
                }
            };
            ...
            function setNextPopOpenTime() {
                if (config.popupCookieExp && (config.popupCookieExp != "")) {
                    ...
                    var dateString = new Date(date.getTime() - (date.getTimezoneOffset() * 60000))
                        .toISOString()
                        .split("T")[0];
                    localStorage.setItem('waqas_popup_expire_date', dateString);
                }
            }

            var popup = modal(options, $('#newsletter-model')),
                CurrentDate = new Date(),
                GivenDate = localStorage.getItem('waqas_popup_expire_date');
            // Open pop after x days
            if (config.popupCookieExp != "" && (getCookie('waqas_custom_popup') === '1')) {
            ...
            if (getCookie('waqas_custom_popup') == null) {
                if((typeof config.popupShowTime !== 'undefined') && (config.popupShowTime)) {
                    // Pop open after x seconds
                    setTimeout(function () {
                        $("#newsletter-model").modal("openModal");
            ...
            $('#waqas-newsletter-validate-detail').submit(function () {
                if ($('#waqas-newsletter-validate-detail').validation('isValid')) {
